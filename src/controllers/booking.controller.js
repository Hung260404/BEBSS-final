...
const bookingController = {
    ...
    checkAvailability: async (req, res) => {
        try {
            ...
            if (!providerId || !serviceId || !date) {
                return res.status(400).json({ error: "Thiếu thông tin providerId, serviceId, date" });
            }
            const slots = await bookingService.checkAvailability({ providerId, serviceId, date });
    ...
    confirm: async (req, res) => {
        try {
            const result = await bookingService.updateStatus(req.user.id, req.params.id, 'CONFIRMED');
    ...
    reject: async (req, res) => {
        try {
            const result = await bookingService.updateStatus(req.user.id, req.params.id, 'REJECTED');
    ...
    complete: async (req, res) => {
        try {
            const result = await bookingService.updateStatus(req.user.id, req.params.id, 'COMPLETED');
    ...
    createSubOrder: async (req, res) => {
        try {
            ...
            const result = await bookingService.createSubOrder(req.user.id, req.params.id, amount, note);
